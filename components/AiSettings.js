...
export function AiSettings() {
  ...
  async function remove() {
    ...
    setForm(empty); setMeta({}); setMessage('已删除')
  }

  const disabled = state === 'loading' || state === 'saving'
  return <section className='settings-section'>
    <header><span>AI</span><h3>模型与 API</h3></header>
    ...
    <form className='settings-form' onSubmit={save}>
      ...
      <label><span>API Key</span><input autoComplete='off' disabled={disabled} type='password' value={form.apiKey} onChange={event => update('apiKey', event.target.value)} placeholder={meta.secretHint ? '留空保留原密钥' : 'sk-…'} /></label>
      <div className='settings-form-grid'>
        ...
        <label><span>日程解析</span><input disabled={disabled} value={form.scheduleModel} onChange={event => update('scheduleModel', event.target.value)} placeholder='默认模型' /></label>
