...
export function CourseTextPackDesk() {
  ...
  return <div className='course-workspace compact'>
    ...
    {view === 'import' ? <section className='course-import-shell'>
      <header className='course-import-header'><div><span>{supplementTarget ? '补充课程资料' : '导入课程资料'}</span><h2>{courseName || '新课程'}</h2><p>{supplementTarget ? '新增材料会匹配到已有课次，也可以新建课次。' : '普通资料在浏览器读取；扫描资料在线识别后只保存文字。'}</p></div><ServiceLights capabilities={capabilities} /></header>
