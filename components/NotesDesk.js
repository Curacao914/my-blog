...
export function NotesDesk() {
    ...
    try {
      ...
      const response = await fetch('/api/notes', {
        ...
        body: JSON.stringify({
          ...
          title,
          bodyMarkdown: body,
          originType: activeNote?.metadata?.originType || activeNote?.metadata?.origin_type || 'quick_note'
        })
      })
  ...
  async function deleteCurrentNote() {
    ...
    setStatus('')
    try {
      const response = await fetch(`/api/notes?id=${encodeURIComponent(activeId)}`, { method: 'DELETE' })
  ...
  return (
    <div className={`notes-desk ${showListOnMobile ? '' : 'show-editor'}`}>
      <aside className="notes-list" aria-label="随手记列表">
        ...
        {notes.length ? (
          notes.map((note) => (
            <button key={note.id} type="button" aria-pressed={activeId === note.id} onClick={() => selectNote(note.id)}>
              <strong>{noteTitle(note)}</strong>
              <small>{excerptText(note.metadata?.excerpt || noteBody(note), 82)}</small>
              <span>{[note.note_type === 'reading' ? '阅读草稿' : '随手记', formatTime(note.updated_at)].filter(Boolean).join(' · ')}</span>
            </button>
          ))
        ) : (
          <p className="notes-empty">还没有草稿</p>
        )}
      </aside>

      <section className="notes-editor" aria-label="随手记编辑">
        <button className="notes-back" type="button" onClick={() => setShowListOnMobile(true)}>
          返回列表
        </button>
        {hasEditor ? (
          <>
