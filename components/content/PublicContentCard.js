  ...
  return <>
    <Link className={`public-content-card public-content-card-window-v2 ${compact ? 'is-compact' : ''} ${featured ? 'is-featured' : ''} ${item?.cover ? 'has-cover' : 'generated-cover'}`} href={publicContentHref(item)}>
      ...
      <div className='public-content-card-body'>
        <div className='public-content-card-meta'><span>{publicContentTypeLabel(item?.type)}</span><span>{category}</span></div>
        <h3>{item?.title || '未命名内容'}</h3>
        {item?.summary ? <p>{item.summary}</p> : null}
        {tags.length ? <div className='public-content-card-tags'>{tags.map(tag => <span key={tag}>{tag}</span>)}</div> : null}
        <footer><span>{formatDate(publicContentDate(item))}</span><b>阅读 ↗</b></footer>
      </div>
    </Link>
    ...
    <style jsx>{`
      ...
      .public-content-card-body { display:flex; min-height:148px; flex-direction:column; padding:14px; }
      ...
      .is-compact .public-content-card-cover { min-height:80px; }
      .is-compact .public-content-card-body { min-height:126px; padding:12px; }
      .is-featured { height:100%; grid-template-rows:minmax(150px,.9fr) auto; }
      .is-featured .public-content-card-cover { min-height:150px; padding:20px; }
      .is-featured .public-content-card-cover strong { font-size:clamp(27px,3vw,40px); }
      .is-featured .public-content-card-body { min-height:172px; padding:18px; }
