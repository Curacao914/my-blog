...
function relaySlug(page = {}) {
  const explicit = String(page.slug || '').trim().replace(/^\/+|\/+$/g, '')
  if (explicit) return explicit
  const stableId = String(page.id || '').replace(/-/g, '')
  return stableId
}

function relayPages(siteData) {
  return (siteData?.allPages || []).filter(page => {
    if (!page?.id || page.slug === 'oops') return false
...
export async function syncNotionRelay({ triggeredBy } = {}) {
  ...
  try {
    for (let index = 0; index < pages.length; index++) {
      ...
      const postData = postDataForStorage({
        ...page,
        slug: relaySlug(page)
      })
      ...
      stats.imageFailures += mirrored.failures.length
    }
