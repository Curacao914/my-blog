...
async function getManagedRecordBySource(sourceId) {
...
export async function getManagedContentBySource(sourceId) {
  return (await getManagedRecordBySource(sourceId))?.managed || null
}

export async function listManagedContent(limit = 100) {
  const safeLimit = Math.min(Number(limit) || 100, 200)
  const items = await request(`/content_items?select=*&order=updated_at.desc&limit=${safeLimit}`)
...
export async function saveCoursePublication({
  ...
  const record = await getManagedRecordBySource(sourceId)
