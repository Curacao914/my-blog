...
export async function ensureProfile({ clerkUserId, email = '', displayName = '', avatarUrl = '', role = 'member', status = 'pending', permissions = {} }) {
  const safeUserId = clerkUserId || 'local-dev'
  const select = 'id,clerk_user_id,email,display_name,avatar_url,role,status,permissions,created_at,updated_at'
  const existing = await supabaseRest(
    `/profiles?select=${select}&clerk_user_id=${eq(safeUserId)}&limit=1`
  )
  if (existing?.[0]) return { profile: existing[0] }

  const created = await supabaseRest(`/profiles?select=${select}`, {
    method: 'POST',
    headers: { Prefer: 'return=representation' },
    body: JSON.stringify({
      clerk_user_id: safeUserId,
...
export async function listScheduleRowsByIds(ids = []) {
  const normalized = [...new Set(
    ids.filter(id => /^[0-9a-f-]{36}$/i.test(String(id || '')))
  )]
  if (!normalized.length) return []
  return supabaseRest(
    `/schedule_items?select=*&id=in.(${normalized.map(encodeURIComponent).join(',')})`
  )
}
...
export function partitionScheduleRows(rows = []) {
  return {
    existingRows: rows.filter(row => Boolean(row?.id)),
...
export async function upsertScheduleRows(rows = []) {
  ...
  const { existingRows, newRows } = partitionScheduleRows(rows)
  const saved = []

  if (existingRows.length) {
    const updated = await supabaseRest(
      '/schedule_items?on_conflict=id&select=*',
      {
        method: 'POST',
        headers: {
  ...
  if (newRows.length) {
    const created = await supabaseRest('/schedule_items?select=*', {
      method: 'POST',
      headers: { Prefer: 'return=representation' },
...
export async function listNotes(ownerId, options = {}) {
  ...
  if (filters.noteType) params.push(`note_type=${eq(filters.noteType)}`)
