...
export async function ensureProfile({ clerkUserId }) {
  ...
  const created = await rest('/profiles?select=id,clerk_user_id', {
    method: 'POST',
...
export async function upsertScheduleRows(rows = []) {
  ...
  return rest('/schedule_items?on_conflict=id&select=*', {
    method: 'POST',
...
export async function listNotes(ownerId, scheduleItemId) {
  const metadataFilter = scheduleItemId
    ? `&metadata=cs.${encodeURIComponent(JSON.stringify({ scheduleItemId }))}`
    : ''
  return rest(
    `/notes?select=id,title,body_markdown,note_type,status,metadata,created_at,updated_at&owner_id=${eq(ownerId)}${metadataFilter}&order=updated_at.desc&limit=${scheduleItemId ? 1 : 30}`
  )
}
...
export async function upsertNote(ownerId, payload, existingId) {
  if (existingId) {
    const rows = await rest(
      `/notes?id=${eq(existingId)}&owner_id=${eq(ownerId)}&select=id,title,body_markdown,note_type,status,metadata,created_at,updated_at`,
      {
        method: 'PATCH',
        headers: { Prefer: 'return=representation' },
  ...
  const rows = await rest(
    '/notes?select=id,title,body_markdown,note_type,status,metadata,created_at,updated_at',
    {
      method: 'POST',
      headers: { Prefer: 'return=representation' },
