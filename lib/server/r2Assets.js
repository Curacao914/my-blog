...
export function isR2StorageConfigured(config = getR2Config()) {
  return Boolean(
    config.endpoint &&
    config.bucket &&
    config.accessKeyId &&
...
export function signR2PutRequest(options) {
  return signR2Request({ ...options, method: 'PUT' })
}
...
export async function getR2Object({ key, config = getR2Config() }) {
  ...
  if (!response.ok) {
    const error = new Error(`R2 read failed (${response.status})`)
...
export async function deleteR2Object({ key, config = getR2Config() }) {
