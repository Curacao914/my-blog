...
const RUN_DETAIL_SELECT = `${RUN_SELECT},results`
...
function structuredActual(actual = {}) {
  const allowed = [
    'version', 'intentId', 'action', 'domain', 'objectType', 'scope',
    'slots', 'contextReferences', 'uncertainties', 'executionAllowed'
  ]
  return Object.fromEntries(
    allowed
      .filter(key => Object.prototype.hasOwnProperty.call(actual, key))
      .map(key => [key, actual[key]])
  )
}

function serializedResults(results = []) {
  const report = buildEvaluationReport(results)
  return {
    report,
    results: report.results.map(item => ({
      caseId: item.caseId,
      actual: structuredActual(item.actual),
      deterministic: item.deterministic,
      modelError: item.modelError
        ? String(item.modelError).slice(0, 300)
        : null,
      budgetExceeded: Boolean(item.budgetExceeded),
      pricingUnknown: Boolean(item.pricingUnknown),
      inputTokens: finiteNumber(item.inputTokens),
      outputTokens: finiteNumber(item.outputTokens),
      estimatedUsd: finiteNumber(item.estimatedUsd),
      latencyMs: finiteNumber(item.latencyMs)
    }))
  }
}
...
export async function getEvaluationRun({ ownerId, environment, runId }) {
  requireOwner(ownerId)
  validateAgentEnvironment(environment)
  const rows = await supabaseRest(
    `/openclaw_agent_eval_runs?select=${RUN_DETAIL_SELECT}` +
    `&id=${eq(runId)}&owner_id=${eq(ownerId)}` +
    `&environment=${eq(environment)}&limit=1`
  )
  return rows?.[0] || null
}

export async function saveEvaluationRunProgress({
  ownerId,
  environment,
  runId,
  expectedCaseCount,
  ...
  requireOwner(ownerId)
  validateAgentEnvironment(environment)
  const serialized = serializedResults(results)
  const rows = await supabaseRest(
    `/openclaw_agent_eval_runs?select=${RUN_DETAIL_SELECT}` +
    `&id=${eq(runId)}&owner_id=${eq(ownerId)}` +
    `&environment=${eq(environment)}&status=eq.running` +
    `&case_count=eq.${Math.max(0, Number(expectedCaseCount) || 0)}`,
    {
      method: 'PATCH',
      headers: { Prefer: 'return=representation' },
      body: JSON.stringify({
        case_count: serialized.results.length,
        usage: serialized.report.usage,
        estimated_cost_usd: serialized.report.usage.estimatedUsd,
        latency_ms: serialized.report.usage.latencyMs,
        results: serialized.results
      })
    }
  )
  if (!rows?.[0]) throw new Error('Evaluation progress changed concurrently')
  return rows[0]
}
