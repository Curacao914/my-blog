...
export async function updateWorkspaceProfile(id, patch = {}) {
  const payload = { updated_at: new Date().toISOString() }
  if ('clerkUserId' in patch) payload.clerk_user_id = String(patch.clerkUserId || '').trim() || null
...
export async function ensureWorkspaceProfile(identity = {}, options = {}) {
  ...
  // Clerk Development and Production instances assign different user IDs.
  // Rebind the existing workspace profile by the verified account email so
  // switching instances does not attempt to insert a duplicate profile.
  if (!profile && email) {
    const emailProfile = await findWorkspaceProfileByEmail(email)
    if (emailProfile) {
      profile = emailProfile.clerk_user_id === clerkUserId
        ? emailProfile
        : await updateWorkspaceProfile(emailProfile.id, { clerkUserId })
    }
  }
