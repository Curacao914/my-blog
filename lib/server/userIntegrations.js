...
export async function upsertUserIntegration(ownerId, provider, input = {}) {
  ...
  const rows = await supabaseRest('/user_integrations?on_conflict=owner_id,provider&select=*', {
    method: 'POST',
...
function ownerGlobalAiConfig() {
  const apiKey = process.env.COURSE_AI_API_KEY || process.env.SCHEDULE_AI_API_KEY || process.env.AI_API_KEY || process.env.OPENAI_API_KEY || ''
  const baseUrl = process.env.COURSE_AI_BASE_URL || process.env.SCHEDULE_AI_BASE_URL || process.env.AI_BASE_URL || 'https://api.openai.com/v1'
  const fallback = process.env.COURSE_AI_MODEL || process.env.SCHEDULE_AI_MODEL || process.env.AI_MODEL || ''
  return {
    apiKey,
    ...
    models: {
      ...
      revision: process.env.COURSE_REVISION_MODEL || process.env.COURSE_WRITER_MODEL || fallback,
...
export async function resolveUserAiConfig(profile) {
  ...
  const record = await getUserIntegration(profile.id, 'openai-compatible')
  if (record?.enabled !== false && record?.secret_ciphertext) {
    const config = record.config || {}
    return {
      apiKey: decryptUserSecret(record),
      baseUrl: String(record.base_url || 'https://api.openai.com/v1').replace(/\/$/, ''),
      models: {
        default: config.defaultModel || '',
        schedule: config.scheduleModel || config.defaultModel || '',
        outline: config.outlineModel || config.defaultModel || '',
        writer: config.writerModel || config.defaultModel || '',
        reviewer: config.reviewerModel || config.defaultModel || '',
        revision: config.revisionModel || config.writerModel || config.defaultModel || '',
        finalReview: config.finalReviewModel || config.defaultModel || ''
      },
      source: 'user'
    }
  }
  if (profile.role === 'owner') return ownerGlobalAiConfig()
  return { apiKey: '', baseUrl: '', models: {}, source: 'missing' }
}
