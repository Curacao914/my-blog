...
export async function createTaskFromCapture(ownerId, rawText, context = {}) {
  if (!ownerId) throw new Error('ownerId is required')
  const task = { ...parseTaskCapture(rawText, context), owner_id: ownerId }
...
export async function listRecentTasks(ownerId, limit = 12) {
  if (!ownerId) throw new Error('ownerId is required')
  return supabaseRequest(
    `/tasks?select=id,owner_id,raw_text,title,status,type,priority,starts_at,due_at,remind_at,place,links,file_refs,source,source_user,source_message_id,attachments,reminder_sent_at,notes,created_at,updated_at&owner_id=eq.${encodeURIComponent(ownerId)}&order=created_at.desc&limit=${limit}`
  )
}

export async function updateTask(ownerId, taskId, patch = {}) {
  if (!ownerId) throw new Error('ownerId is required')
  const id = String(taskId || '').trim()
  if (!id) throw new Error('taskId is required')

  const payload = normalizeTaskPatch(patch)
  const rows = await supabaseRequest(`/tasks?id=eq.${encodeURIComponent(id)}&owner_id=eq.${encodeURIComponent(ownerId)}`, {
    method: 'PATCH',
    headers: {
      Prefer: 'return=representation'
    },
