...
export async function listTextPackCourseJobs(ownerId, limit = 20) {
  ...
  const jobs = await supabaseRequest(
    `/course_jobs?select=id,owner_id,course_name,lesson,teacher,status,current_node,preferences,preprocess_reported_at,created_at,updated_at&owner_id=eq.${encodeURIComponent(ownerId)}&preferences=cs.${ownerFilter}&order=updated_at.desc&limit=${Math.min(Number(limit) || 20, 60)}`
  )
...
function migratedLessonStatus(lesson, nodes) {
  ...
  if (nodes.every(node => node.status === 'node_approved') && lesson.finalNote?.markdown) {
    const reports = Array.isArray(lesson.finalReviewReports) ? lesson.finalReviewReports : []
    const latest = reports.at(-1)?.value || lesson.qualityReport || lesson.finalNote?.qualityReport || null
    const versions = Array.isArray(lesson.finalNoteVersions) ? lesson.finalNoteVersions : []
    const currentReport = Boolean(latest) && (
      latest.reviewedDraftVersion === undefined || latest.reviewedDraftVersion === null
        ? lesson.status === 'final_review_human'
        : Number(latest.reviewedDraftVersion) === Number(versions.length || 0)
    )
    const requiresHuman = Boolean(lesson.finalReviewAttention) || (currentReport && latest?.decision === 'human_review')
    return requiresHuman ? 'final_review_human' : 'final_review'
  }
...
export function workflowFromJob(job) {
  ...
  const lessons = (workflow.lessons || []).map(lesson => {
    const nodes = (lesson.nodes || []).map(node => {
      ...
      const genuinelyRequiresHuman = Boolean(
        latest?.requestedDecision === 'human_review' ||
        latest?.autoRevisionExhausted === true ||
        latestIssues.some(issue => typeof issue === 'object' && issue?.requiresHuman)
      )
      if (migratedStatus === 'node_human_review' && !genuinelyRequiresHuman && revisionCount < maxAutoRevisions) migratedStatus = 'node_revision_required'
      if (migratedStatus === 'node_revision_required' && (genuinelyRequiresHuman || revisionCount >= maxAutoRevisions)) migratedStatus = 'node_human_review'
    ...
    return {
      ...
      finalReviewAttention: lesson.finalReviewAttention || null,
  ...
  const normalizedResumeStatus = workflow.resumeStatus === 'final_review' && activeLesson?.status === 'final_review_human'
    ? 'final_review_human'
    : (workflow.resumeStatus || null)
  const manualFinalReview = activeLesson?.status === 'final_review_human'
  ...
  return {
    ...
    taskLease: manualFinalReview ? null : (workflow.taskLease || null),
    appliedTaskKeys: Array.isArray(workflow.appliedTaskKeys) ? workflow.appliedTaskKeys : [],
    taskLeases: manualFinalReview ? [] : storedLeases,
    courseSpec: {
      qualityThreshold: 75,
