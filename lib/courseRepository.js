...
export async function listCourseNoteLibrary(ownerId, limit = 40) {
  ...
  const jobs = await supabaseRequest(
    `/course_jobs?select=id,course_name,teacher,status,current_node,preferences,preprocess_result,updated_at&preferences=cs.${ownerFilter}&order=updated_at.desc&limit=${Math.min(Number(limit) || 40, 80)}`
  )
