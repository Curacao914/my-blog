...
function migratedLessonStatus(lesson, nodes) {
  ...
  if (nodes.every(node => node.status === 'node_approved') && lesson.finalNote?.markdown) return 'final_review_human'
...
function reduceWorkerAction(workflow, action = {}) {
  ...
  if (type === 'heartbeat') {
    ...
    if (workflow.status === 'completed' || ['final_review_human', 'completed'].includes(lesson?.status)) {
      nextWorkflow = workflow
      note = '迟到的自动最终检查结果已忽略，最终内容由用户确认。'
    } else {
