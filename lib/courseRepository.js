...
export async function createCourseJob(ownerId, input) {
  if (!ownerId) throw new Error('ownerId is required')
  const job = { ...normalizeJobInput(input), owner_id: ownerId }
...
export async function listCourseJobs(ownerId, limit = 6) {
  if (!ownerId) throw new Error('ownerId is required')
  const jobs = await supabaseRequest(
    `/course_jobs?select=id,owner_id,course_name,lesson,teacher,status,current_node,preferences,preprocess_result,preprocess_reported_at,local_workdir,material_bundle_confirmed_at,preflight_confirmed_at,created_at,updated_at&owner_id=eq.${encodeURIComponent(ownerId)}&order=created_at.desc&limit=${limit}`
  )
...
export async function listTextPackCourseJobs(ownerId, limit = 20) {
  ...
  const jobs = await supabaseRequest(
    `/course_jobs?select=id,owner_id,course_name,lesson,teacher,status,current_node,preferences,preprocess_result,preprocess_reported_at,created_at,updated_at&owner_id=eq.${encodeURIComponent(ownerId)}&preferences=cs.${ownerFilter}&order=updated_at.desc&limit=${Math.min(Number(limit) || 20, 60)}`
  )
...
export async function getCourseJobById(jobId, ownerId = '') {
  const ownerFilter = ownerId ? `&owner_id=eq.${encodeURIComponent(ownerId)}` : ''
  const rows = await supabaseRequest(
    `/course_jobs?select=id,owner_id,course_name,lesson,teacher,run_mode,status,current_node,preferences,preprocess_result,preprocess_reported_at,local_workdir,material_bundle_confirmed_at,preflight_confirmed_at,changelog,created_at,updated_at&id=eq.${encodeURIComponent(jobId)}${ownerFilter}&limit=1`
  )
...
async function findExistingTextPackJob(ownerId, textPack) {
  ...
  const rows = await supabaseRequest(
    `/course_jobs?select=id,owner_id,course_name,lesson,teacher,status,current_node,preferences,preprocess_result,preprocess_reported_at,created_at,updated_at&owner_id=eq.${encodeURIComponent(ownerId)}&preferences=cs.${ownerFilter}&order=updated_at.desc&limit=1`
  )
  return rows?.[0] || null
}
...
export async function deleteTextPackCourseJob(ownerId, jobId) {
  ...
  const ownerProfileId = job.owner_id || job.preferences?.web_adapter?.ownerProfileId
...
function migratedLessonStatus(lesson, nodes) {
  if (lesson.status === 'note_removed') return 'note_removed'
  if (lesson.status === 'completed') return 'completed'
  if (lesson.status === 'final_revision_required') return 'final_revision_required'
  if (!nodes.length) return lesson.status
  if (nodes.every(node => node.status === 'node_approved') && lesson.finalNote?.markdown) {
    return 'final_review'
  }
...
export function workflowFromJob(job) {
  ...
  const lessons = (workflow.lessons || []).map(lesson => {
    const nodes = (lesson.nodes || []).map(node => {
      ...
      let migratedStatus = node.status === 'node_review' && currentReview && latest?.decision === 'approve' ? 'node_approved'
        : node.status === 'node_review' && currentReview && ['revise', 'human_review'].includes(latest?.decision) ? 'node_revision_required'
          : node.status
      const revisionCount = Number(node.revisionCount || 0)
      const storedReviewStatus = migratedStatus
      const contentReviewExhausted =
        ['node_human_review', 'node_revision_required'].includes(storedReviewStatus) &&
        revisionCount >= maxAutoRevisions
      if (migratedStatus === 'node_human_review') {
        migratedStatus = revisionCount < maxAutoRevisions ? 'node_revision_required' : 'node_approved'
      }
      if (migratedStatus === 'node_revision_required' && revisionCount >= maxAutoRevisions) {
        migratedStatus = 'node_approved'
      }
      ...
      return {
        ...
        reviewDecision: migratedStatus === 'node_approved' ? 'approve' : (node.reviewDecision || latest?.decision || null),
        revisionRequests,
        reviewerReports,
        versions,
        ...
        blocksDownstream: migratedStatus === 'node_approved' ? false : Boolean(node.blocksDownstream),
        consistencyRequests: Array.isArray(node.consistencyRequests) ? node.consistencyRequests : [],
        autoAcceptedWithWarnings: Boolean(node.autoAcceptedWithWarnings || contentReviewExhausted),
        humanReviewRequired: migratedStatus === 'node_failed'
      }
    })
    return {
      ...lesson,
      ...
      finalRevisionRequests: Array.isArray(lesson.finalRevisionRequests) ? lesson.finalRevisionRequests : [],
      finalRevisionCount: Number(lesson.finalRevisionCount || 0),
      finalReviewAttention: null,
      noteDeletion: lesson.noteDeletion?.deletedAt ? lesson.noteDeletion : null,
      notePurgedAt: lesson.notePurgedAt || null,
      publication: lesson.publication || null,
      brief: lesson.brief || null,
      briefVersions: Array.isArray(lesson.briefVersions) ? lesson.briefVersions : [],
  ...
  const controlStatuses = new Set(['paused', 'failed', 'cancelled', 'completed'])
  const normalizedStatus = activeLesson && !controlStatuses.has(workflow.status) ? activeLesson.status : workflow.status
  const normalizedResumeStatus = workflow.resumeStatus === 'final_review_human'
    ? 'final_review'
    : workflow.resumeStatus === 'node_human_review'
      ? 'node_revision_required'
      : (workflow.resumeStatus || null)
  const migratedContentGate =
    ['node_human_review', 'final_review_human'].includes(workflow.status) ||
    (workflow.lessons || []).some(lesson => ['node_human_review', 'final_review_human'].includes(lesson.status))
  const storedLeases = Array.isArray(workflow.taskLeases) ? workflow.taskLeases : (workflow.taskLease ? [workflow.taskLease] : [])
  return {
    ...workflow,
    schemaVersion: 'course-workflow.v2',
    status: normalizedStatus,
    resumeStatus: normalizedResumeStatus,
    taskLease: migratedContentGate ? null : (workflow.taskLease || null),
    appliedTaskKeys: Array.isArray(workflow.appliedTaskKeys) ? workflow.appliedTaskKeys : [],
    taskLeases: migratedContentGate ? [] : storedLeases,
    courseSpec: {
      qualityThreshold: 75,
...
export async function listCourseNoteLibrary(ownerId, limit = 40) {
  const ownerFilter = encodeURIComponent(
    JSON.stringify({ web_adapter: { ownerProfileId: ownerId, source: 'textpack-v1' } })
  )
  const jobs = await supabaseRequest(
    `/course_jobs?select=id,course_name,teacher,status,current_node,preferences,preprocess_result,updated_at&owner_id=eq.${encodeURIComponent(ownerId)}&preferences=cs.${ownerFilter}&order=updated_at.desc&limit=${Math.min(Number(limit) || 40, 80)}`
  )
  return (jobs || []).flatMap(job => {
    try {
      return [buildCourseNoteLibraryEntry(job, workflowFromJob(job))]
    } catch {
      return []
    }
  })
...
export async function getCourseNoteForOwner(ownerId, jobId, lessonKey) {
  const job = await getTextPackCourseJobForOwner(ownerId, jobId)
  return buildCourseNoteReaderEntry(job, workflowFromJob(job), lessonKey)
}


function traceFilter(value = {}) {
  return encodeURIComponent(JSON.stringify(value))
}

async function findScheduleItemByTrace(ownerId, value = {}) {
  const rows = await supabaseRequest(
    `/schedule_items?select=*&owner_id=eq.${encodeURIComponent(ownerId)}&ai_trace=cs.${traceFilter(value)}&limit=1`
  )
  return rows?.[0] || null
}

async function insertScheduleItem(row) {
  const rows = await supabaseRequest('/schedule_items', {
    method: 'POST',
    headers: { Prefer: 'return=representation' },
...
async function updateScheduleItem(id, ownerId, patch) {
  const rows = await supabaseRequest(
    `/schedule_items?id=eq.${encodeURIComponent(id)}&owner_id=eq.${encodeURIComponent(ownerId)}`,
    {
      method: 'PATCH',
      headers: { Prefer: 'return=representation' },
...
async function ensureCourseBriefFolder(ownerId, courseName) {
  let root = await findScheduleItemByTrace(ownerId, {
    entityType: 'reading-folder',
    systemKey: 'course-briefs'
  })
  if (!root) {
    root = await insertScheduleItem({
      owner_id: ownerId,
      title: '课程简报',
      section: '阅读',
      section_key: 'reading',
  ...
  const courseKey = crypto
    .createHash('sha256')
    .update(String(courseName || '未命名课程'))
    .digest('hex')
    .slice(0, 20)
  const captureKey = `course-brief-folder:${courseKey}`
  let courseFolder = await findScheduleItemByTrace(ownerId, { captureKey })
  if (!courseFolder) {
    courseFolder = await insertScheduleItem({
      owner_id: ownerId,
      title: courseName || '未命名课程',
      section: '阅读',
      section_key: 'reading',
      schedule_date: 'reading',
      status: 'active',
      links: [],
      children: [],
      summary: `${courseName || '课程'}的课程简报`,
      note: '',
      source: 'course-automation',
      ai_trace: {
        entityType: 'reading-folder',
        contentType: 'reading',
        folderName: courseName || '未命名课程',
...
async function upsertCourseBriefReadingItem({
  ownerId,
  jobId,
  workflow,
  lesson,
  ...
  const courseName = workflow.courseSpec?.courseName || '未命名课程'
  const folder = await ensureCourseBriefFolder(ownerId, courseName)
  const captureKey = `course-brief:${jobId}:${lesson.key}`
  const existing = await findScheduleItemByTrace(ownerId, { captureKey })
  const siteUrl = String(
    process.env.NEXT_PUBLIC_SITE_URL ||
    process.env.NEXT_PUBLIC_LINK ||
    'https://law-tech.dev'
  ).replace(/\/$/, '')
  const objectUrl = `/desk/materials/${jobId}/${encodeURIComponent(lesson.key)}`
  const patch = {
    ...
    links: [
      { title: '打开完整课程笔记', url: objectUrl }
    ],
    children: existing?.children || [],
    summary: brief.mainLine || '',
    note: existing?.note || '',
    source: 'course-automation',
    ai_trace: {
      ...(existing?.ai_trace || {}),
      ...
      contentType: 'reading',
      readingKind: 'course-brief',
      bodyMarkdown: brief.markdown,
      folderId: folder.id,
      captureKey,
      courseName,
      courseJobId: jobId,
      ...
      fullNoteUrl: objectUrl,
      absoluteUrl: `${siteUrl}${objectUrl}`,
      generatedAt: new Date().toISOString()
    }
  }
  if (existing) return updateScheduleItem(existing.id, ownerId, patch)
  return insertScheduleItem({ owner_id: ownerId, ...patch })
}

export async function persistCourseBriefArtifact({
  jobId,
  ownerId,
  workflow,
  lessonKey,
  ...
  const taskKey = `course-brief:${lessonKey}:${workflow.lessons?.find(item => item.key === lessonKey)?.finalNoteVersions?.length || 0}`
  const stored = await applyCourseWorkflowActionsForWorker(jobId, [{
    type: 'save-course-brief',
    lessonKey,
    brief,
    trace,
    taskKey
  }])
  const lesson = stored.workflow.lessons?.find(item => item.key === lessonKey)
  const readingItem = await upsertCourseBriefReadingItem({
    ownerId,
    jobId,
  ...
  const integration = await getWechatIntegration(ownerId)
  const preference = publicWechatPreference(integration)
  let delivery = null
  if (preference.enabled && preference.courseBriefEnabled) {
    const objectUrl = `/desk/reading`
    const scheduledFor = nextWechatDeliveryTime({
      mode: preference.courseBriefDelivery,
      time: preference.courseBriefTime,
      timezone: preference.timezone
    })
    delivery = await enqueueMessageDelivery({
      ownerId,
      purpose: 'course-brief',
      dedupeKey: `course-brief:${jobId}:${lessonKey}`,
      subject: brief.title,
      bodyText: [
        brief.messageText,
        '',
      ...
      objectType: 'course-brief',
      objectId: readingItem?.id || `${jobId}:${lessonKey}`,
      objectUrl,
      scheduledFor,
      metadata: {
        jobId,
        lessonKey,
        courseName: stored.workflow.courseSpec?.courseName || '',
        deliveryMode: preference.courseBriefDelivery
      }
    })
  }
...
export function courseRuntimeSummary(workflow = {}) {
  ...
  return {
    ...
    counts: {
      ...
      attention: Number(counts.node_human_review || 0) + Number(counts.node_failed || 0) + (activeLesson?.status === 'final_review_human' ? 1 : 0),
...
export async function getTextPackCourseRuntimeForOwner(ownerId, jobId) {
  ...
  const rows = await supabaseRequest(
    `/course_jobs?select=id,course_name,status,current_node,preferences,updated_at&id=eq.${encodeURIComponent(jobId)}&owner_id=eq.${encodeURIComponent(ownerId)}&preferences=cs.${ownerFilter}&limit=1`
  )
  const job = rows?.[0]
  if (!job) throw new Error('Course job not found')
  return {
...
export async function getCourseRuntime(jobId) {
  const rows = await supabaseRequest(
    `/course_jobs?select=id,owner_id,status,current_node,preferences,updated_at&id=eq.${encodeURIComponent(jobId)}&limit=1`
  )
  const job = rows?.[0]
  if (!job) throw new Error('Course job not found')
  return {
...
export async function applyCourseWorkflowAction(ownerId, jobId, action = {}) {
  ...
  if (type === 'save-course-spec') {
    ...
    note = '最终笔记的人工修改已保存。'
  } else if (type === 'request-final-revision') {
    nextWorkflow = requestFinalNoteRevision(workflow, action.lessonKey, action.request || '')
    note = '最终笔记修改要求已保存，等待按要求修订。'
  } else if (type === 'trash-lesson-note') {
    nextWorkflow = trashLessonNote(workflow, action.lessonKey)
    note = '课次笔记已移入回收站，正文和版本仍然保留。'
  } else if (type === 'restore-lesson-note') {
    nextWorkflow = restoreLessonNote(workflow, action.lessonKey)
    note = '课次笔记已从回收站恢复。'
  } else if (type === 'purge-lesson-note') {
    nextWorkflow = purgeLessonNote(workflow, action.lessonKey)
    note = '课次最终笔记及其最终版本已永久删除，原始材料、大纲和节点正文仍然保留。'
  } else if (type === 'regenerate-lesson-note') {
    nextWorkflow = regenerateLessonNote(workflow, action.lessonKey)
    note = '课次最终笔记已进入重新拼装队列。'
  } else if (type === 'set-lesson-publication') {
    nextWorkflow = setLessonPublication(workflow, action.lessonKey, action.publication || {})
    note = '课程笔记发布关联已更新。'
...
function reduceWorkerAction(workflow, action = {}) {
  ...
  } else if (type === 'approve-outline-worker') {
    nextWorkflow = approveOutline(workflow, action.lessonKey); note = '课程处理服务已按自动化设置批准大纲。'
  ...
  } else if (type === 'save-final-note-revision') {
    nextWorkflow = saveFinalNoteRevision(workflow, action.lessonKey, action.markdown, { trace: action.trace || null })
    note = '课程处理服务已按用户要求修订最终笔记。'
  } else if (type === 'save-course-brief') {
    nextWorkflow = saveCourseBrief(
      workflow,
      action.lessonKey,
      action.brief || {},
      { trace: action.trace || null }
    )
    note = '课程简报已生成并进入阅读资料库。'
  } else if (type === 'complete-final-review') {
    const lesson = workflow.lessons?.find(item => item.key === action.lessonKey)
    if (workflow.status === 'completed' || lesson?.status === 'completed') {
      nextWorkflow = workflow
      note = '迟到的自动最终检查结果已忽略，课程已经完成。'
    } else {
      nextWorkflow = completeFinalReview(workflow, action.lessonKey, { ...(action.qualityReport || action.report || {}), trace: action.trace || null })
      note = '课程处理服务已完成最终检查。'
    }
...
export async function uploadCourseAsset(ownerId, jobId, file) {
  const job = await getCourseJobById(jobId, ownerId)
  if (!job) {
    throw new Error('Course job not found')
  }
