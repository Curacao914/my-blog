...
function buildTextPackPreprocessResult(textPack, summary) {
  ...
  return {
    ...
    next: summary.ocrRequired
      ? 'TextPack 已导入；存在低文本密度课件，后续需要 OCR 或补充文字。'
      : 'TextPack 已导入；可进入 preflight 与大纲设计。',
    textPack,
    ...
    pptNeedsOcr: textPack.ppt_text
      ...
      .map(deck => ({
        deck: deck.title,
        key: deck.key,
        note: '低文本密度 PPTX，TextPack 阶段不自动 OCR。'
      })),
    failures: [],
    workflow
  }
      ...
      changelog: [
        {
          ...
          note: `导入 TextPack：${summary.lessonCount} 课，${summary.deckCount} 份课件，${summary.totalChars} 字符。`
        }
      ]
    })
  })
        ...
        changelog: [
          {
            ...
            note: `${lesson.sourceFile} 已作为纯文本转录导入。`
          }
        ],
        updated_at: now
      }))
...
async function patchCourseWorkflow(job, workflow, event, note) {
  ...
  const rows = await supabaseRequest(`/course_jobs?id=eq.${encodeURIComponent(job.id)}`, {
    ...
    body: JSON.stringify({
      ...
      changelog: appendChangelog(job, event, note)
    })
  })
...
export async function applyCourseWorkflowAction(ownerId, jobId, action = {}) {
  ...
  if (type === 'save-course-spec') {
    ...
    note = '课程偏好已保存，preflight 已确认。'
  } else if (type === 'save-outline') {
    nextWorkflow = startOutlineReview(workflow, {
    ...
    nextWorkflow = updateOutline(workflow, action.lessonKey, (node, index) => {
      const patch = (action.outline || [])[index] || node
      return {
        ...node,
        ...patch,
        userEdited: true
    ...
    nextWorkflow = planNodesFromOutline(workflow, action.lessonKey)
    note = '节点写作任务已按大纲拆分。'
  } else if (type === 'update-node-draft') {
    nextWorkflow = updateNodeDraft(workflow, action.lessonKey, action.nodeId, {
      markdown: action.markdown,
      reviewerReport: action.reviewerReport,
      revisionRequest: action.revisionRequest
    })
    note = '节点草稿或审查结果已保存。'
    ...
    nextWorkflow = assembleFinalNote(workflow, action.lessonKey)
    note = '单课最终 Markdown 已拼装，等待最终检查。'
  } else if (type === 'complete-final-review') {
    nextWorkflow = completeFinalReview(workflow, action.lessonKey, action.qualityReport || action.report || {})
    note = '最终检查已通过，单课笔记完成。'
  } else if (type === 'pause') {
    nextWorkflow = pauseWorkflow(workflow)
    note = '课程流程已暂停领取后续任务。'
  } else if (type === 'resume') {
    nextWorkflow = resumeWorkflow(workflow)
    note = '课程流程已恢复。'
  } else if (type === 'cancel') {
    nextWorkflow = cancelWorkflow(workflow, action.reason || '')
    note = '课程流程已取消，已有文本版本保留。'
  } else if (type === 'fail-step') {
    nextWorkflow = failStep(workflow, {
      step: action.step,
      error: action.error,
    ...
    throw new Error('Unsupported course workflow action')
  }
...
export async function applyCourseWorkflowActionForWorker(jobId, action = {}) {
  ...
  if (type === 'save-outline') {
    ...
    note = 'Worker 已回写节点草稿和审查报告。'
  } else if (type === 'assemble') {
    nextWorkflow = assembleFinalNote(workflow, action.lessonKey)
    note = 'Worker 已拼装最终笔记，等待最终检查。'
  } else if (type === 'complete-final-review') {
    nextWorkflow = completeFinalReview(workflow, action.lessonKey, action.qualityReport || action.report || {})
    note = 'Worker 已完成最终检查。'
  } else if (type === 'fail-step') {
    ...
    note = 'Worker 执行失败，错误已记录。'
  } else {
    throw new Error('Unsupported worker workflow action')
  }

  const updated = await patchCourseWorkflow(job, nextWorkflow, `worker-${type}`, note)
  return {
    job: updated,
    workflow: nextWorkflow
  }
