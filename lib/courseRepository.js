...
function workflowFromJob(job) {
  ...
  return {
    ...
    worker: { status: 'offline', lastSeenAt: null, message: '本地处理服务未连接', ...(workflow.worker || {}) },
...
export async function claimCourseWorkerTask(jobId, leaseSeconds = 180) {
  ...
  const nextWorkflow = {
    ...
    worker: { ...(workflow.worker || {}), status: 'online', lastSeenAt: new Date(now).toISOString(), message: '本地处理服务正在工作' }
  }
...
export async function applyCourseWorkflowActionForWorker(jobId, action = {}) {
  ...
  let note = ''
  if (type === 'heartbeat') {
    nextWorkflow = { ...workflow, worker: { status: 'online', lastSeenAt: new Date().toISOString(), message: String(action.message || '本地处理服务已连接') }, updatedAt: new Date().toISOString() }
  } else if (type === 'save-outline') {
    ...
    nextWorkflow = failStep(workflow, { step: action.step, error: action.error, retryable: action.retryable !== false }); note = '课程处理服务发生技术错误，错误已记录。'
  ...
  if (type !== 'heartbeat') {
    nextWorkflow = { ...nextWorkflow, taskLease: null,
      ...
      worker: { ...(nextWorkflow.worker || {}), status: 'online', lastSeenAt: new Date().toISOString(), message: '本地处理服务正在工作' } }
  }
