...
export class Validator {
  ...
  static isValidUrl(url) {
    ...
    try {
      ...
      if (REGEX_PATTERNS.ipAddress.test(hostname)) return true
      return hostname.includes('.') && hostname.split('.').every(label => /^[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$/i.test(label))
    } catch {
      return false
    }
  }
...
export class Sanitizer {
  ...
  static sanitizeXss(str) {
    ...
    if (/^(?:javascript|vbscript):/i.test(value) || /^data:text\/html/i.test(value)) return ''
