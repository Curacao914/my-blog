...
export function selectRelevantItems(command, items, limit = 20) {
  ...
  if (latestReadingOnly) {
    const readingItems = active.filter((item) => item.contentType === 'reading' || item.section === '阅读' || item.date === 'reading')
  ...
  const scored = active.map((item) => {
    ...
    const score =
      ...
      (wantsReading && (item.contentType === 'reading' || item.section === '阅读' || item.date === 'reading') ? 3 : 0)
    return { item, score }
  })
