...
export function toDbScheduleItem(item, ownerId) {
  ...
  const aiTrace = {
    ...
    ...(Array.isArray(item.reminders) && item.reminders.length ? { reminders: item.reminders } : {}),
    ...(item.temporal ? { temporal: item.temporal } : {}),
    ...(item.recurrence ? { recurrence: item.recurrence } : {}),
    ...(item.commandProtocol ? { commandProtocol: item.commandProtocol } : {}),
  ...
  if (Object.prototype.hasOwnProperty.call(item, 'reminder')) {
    if (item.reminder) aiTrace.reminder = item.reminder
    else delete aiTrace.reminder
  }
  if (Object.prototype.hasOwnProperty.call(item, 'reminders')) {
    if (Array.isArray(item.reminders) && item.reminders.length) {
      aiTrace.reminders = item.reminders
      if (!aiTrace.reminder) aiTrace.reminder = item.reminders[0]
    } else {
      delete aiTrace.reminders
      if (!item.reminder) delete aiTrace.reminder
    }
  }
  if (Object.prototype.hasOwnProperty.call(item, 'temporal')) {
    if (item.temporal) aiTrace.temporal = item.temporal
    else delete aiTrace.temporal
  }
  if (Object.prototype.hasOwnProperty.call(item, 'recurrence')) {
    if (item.recurrence) aiTrace.recurrence = item.recurrence
    else delete aiTrace.recurrence
  }
...
export function fromDbScheduleItem(row) {
  ...
  const reminder = aiTrace.reminder
  const reminders = Array.isArray(aiTrace.reminders)
    ? aiTrace.reminders
    : reminder
      ? [reminder]
      : []
  ...
  return {
    ...
    reminder,
    reminders,
    temporal: aiTrace.temporal || null,
    recurrence: aiTrace.recurrence || null,
    commandProtocol: aiTrace.commandProtocol || null,
    contentType,
