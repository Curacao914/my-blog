...
function minutesBeforeForItem(item) {
  if (item?.reminder?.enabled === false) return null
  if (Number.isFinite(item?.reminder?.leadMinutes)) return Math.max(0, Number(item.reminder.leadMinutes))
  if (item.priority === 'high') return 60
  if (item.contentType === 'reading' || item.sectionKey === 'reading' || item.section === '阅读' || item.date === 'reading') return 30
  return 15
}
...
export function buildReminderForItem(item, ownerId) {
  ...
  return {
    ...
    payload: {
      ...
      leadMinutes: explicitReminderAt ? null : leadMinutes
    }
  }
}
