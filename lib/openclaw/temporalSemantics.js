...
function resolveClock(clock, { date, nowParts, todayKey, explicitDate }) {
  ...
  if (clock.explicitPeriod) {
    const hour = normalizePeriodHour(clock.hour, clock.period)
    return { hour, minute: normalizedMinute, resolution: 'explicit-period' }
  }

  const candidates = [clock.hour]
  if (clock.hour >= 1 && clock.hour <= 11) candidates.push(clock.hour + 12)
  const unique = [...new Set(candidates.filter(hour => hour >= 0 && hour <= 23))]
  if (date && date !== todayKey) {
    return { hour: unique[0], minute: normalizedMinute, resolution: 'bare-clock' }
  }
  ...
  if (future.length) return { ...future[0], resolution: 'same-day-future' }
