...
const DOMAIN_RULES = [
  ...
  ['schedule', /日程|安排|事项|待办|会议|活动|面试|打卡|吃饭|提醒|今天|明天|后天|周[一二三四五六日天]|星期[一二三四五六日天]|\d{1,2}[点:：]/]
]
...
function inferAction(text, domain) {
  ...
  if (DELETE_WORDS.test(text)) return 'delete'
  ...
  if (/^(提前|延后|推迟|改到|改成|提醒)/.test(text)) return 'update'
  if (QUERY_WORDS.test(text)) return domain === 'content' ? 'search' : 'list'
  if (REMINDER_WORDS.test(text) || CREATE_WORDS.test(text) || isStandaloneCreate(text)) return 'create'
