...
function inferDomain(text) {
  ...
  if (/https?:\/\//i.test(text)) return 'reading'
...
function isStandaloneActionPhrase(text) {
  return /^(?:去)?(?:买|写|交|做|处理|联系|回复|准备|整理|复习|背|提交|打印|取|寄|预约|缴费|支付|打电话|发邮件|阅读|读(?!完)|看(?!完)).{1,50}$/.test(text)
}
...
function inferAction(text, domain) {
  ...
  if (/https?:\/\//i.test(text)) return 'create'
  if (
