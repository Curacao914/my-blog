...
function hasOrdinalReference(text) {
  return /(?:第?\d+|第?[一二两三四五六七八九十]+)(?:项|个|条|篇|份)?/.test(String(text || ''))
}

function typedResultSet(expected = {}) {
...
function evaluationSessionState(item = {}) {
  ...
  if (expected.action === 'select') {
    return { resultSet: typedResultSet(expected) }
  }
  if (
    expected.scope === 'single' &&
    expected.domain &&
    expected.objectType &&
    hasOrdinalReference(item.input)
  ) {
    return { resultSet: typedResultSet(expected) }
  }
