...
const INTENT_SERIALIZER = 'emit_user_intent'
...
function parseContent(data) {
  ...
  const candidate = call?.function?.arguments
  if (
    call?.function?.name !== INTENT_SERIALIZER ||
...
function completionUrl(baseUrl) {
  try {
    ...
    // The caller still receives a normal provider error for malformed URLs.
  }
  ...
  try {
    response = await fetchImpl(completionUrl(baseUrl), {
      ...
      body: JSON.stringify({
        ...
        tool_choice: {
          type: 'function',
          function: { name: INTENT_SERIALIZER }
        },
