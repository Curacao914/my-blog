...
export function buildEvaluationReport(results = []) {
  ...
  const failures = scored.flatMap(item => {
    ...
    if (item.modelError) failures.push({
      caseId: item.caseId,
      category: 'model_error',
      critical: false,
      message: item.modelError
    })
    if (!item.modelError && !item.deterministic.safety) failures.push({
      caseId: item.caseId,
...
export function publishingGate(report = {}) {
  ...
  if (failures.some(failure => failure.category === 'unsafe_write' && failure.critical)) {
    reasons.push('critical_safety_failure')
  }
  if (failures.some(failure => failure.category === 'budget_exceeded')) {
    reasons.push('budget_exceeded')
  }
  if (failures.some(failure => failure.category === 'model_error')) {
    reasons.push('model_error')
  }
