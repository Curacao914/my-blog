...
function selectionOrdinal(text) {
  const compact = String(text || '')
    ...
    .replace(/\s+/g, '')
    .replace(/[，,。.!！?？]/g, '')
  const ordinal = '(\\d+|[一二两三四五六七八九十]+)'
  const direct = compact.match(new RegExp(`^(?:就选|选择|选)?第?${ordinal}(?:项|个|条|篇|份)?$`))
  if (direct) return numberFromOrdinal(direct[1])
  const correction = compact.match(new RegExp(`^不是第?${ordinal}(?:项|个|条|篇|份)?是第?${ordinal}(?:项|个|条|篇|份)?$`))
  if (correction) return numberFromOrdinal(correction[2])
  const tentative = compact.match(new RegExp(`^可能是第?${ordinal}(?:项|个|条|篇|份)?(?:先别动|别动)?$`))
