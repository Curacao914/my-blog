...
const boundedText = (maxLength = 300) => ({
  type: 'string',
  minLength: 1,
  maxLength
})
...
export const USER_INTENT_JSON_SCHEMA = Object.freeze({
  ...
  properties: {
    version: { type: 'string', const: AGENT_V2_CONTRACT_VERSION },
    intentId: boundedText(100),
...
export function validateUserIntent(input) {
  ...
  array(value.slots.additionalActions, 'UserIntent.slots.additionalActions')
    .forEach(action => {
      if (!ACTIONS.has(action)) {
        throw new Error('UserIntent.slots.additionalActions contains unsupported action')
      }
    })
  array(value.slots.values, 'UserIntent.slots.values').forEach(slot => {
