...
export function createInitialWorkflow({ textPack, courseName, teacher }) {
  ...
  return {
    ...
    lessons: (textPack.lessons || []).map(lesson => ({
      ...
      outline: [], outlineVersions: [], outlineStale: false, nodes: [], brief: null, briefVersions: [], finalNote: null,
...
export function applyNodeReview(workflow, lessonKey, nodeId, report = {}, { trace = null } = {}) {
  ...
  const next = mapLesson(workflow, lessonKey, lesson => {
    ...
    let nodes = (lesson.nodes || []).map(node => {
      ...
      downstreamImpact = decision === 'revise' && issuesAffectDownstream(finalReport)
      const base = {
        ...node,
      ...
      return {
        ...
        revisionRequests: versioned({
          message: finalReport.issues.map(issue => issue.message).filter(Boolean).join('；') || finalReport.summary || '审查发现需要修正的内容问题。',
...
export function saveCourseBrief(workflow, lessonKey, brief = {}, { trace = null } = {}) {
  const markdown = cleanText(brief.markdown || '')
  if (!markdown) throw new Error('course brief markdown is required')
  return withDerivedProgress(mapLesson(workflow, lessonKey, item => ({
    ...item,
    brief: {
      ...brief,
    ...
    briefVersions: versioned(markdown, item.briefVersions, {
      source: 'course-brief',
      trace,
      summary: cleanText(brief.mainLine || brief.summary || '')
    }),
    updatedAt: nowIso()
  })))
}
  ...
  if (decision === 'revise') {
    ...
    if (!targetIds.length) {
      ...
      if (completedRevisions >= maxRevisions) {
        ...
        return finishLessonWithFinalReport(workflow, lessonKey, acceptedReport, { source: 'worker' })
      }
    ...
    const maxNodeRevisions = Math.max(0, Number(workflow.courseSpec?.maxAutoRevisions ?? DEFAULT_COURSE_SPEC.maxAutoRevisions))
    ...
    const next = mapLesson(workflow, lessonKey, item => ({
      ...
      status: 'node_revision_required',
      finalReviewAttention: null,
      finalNote: { ...item.finalNote, stale: true, qualityReport: reviewForRevision },
      finalReviewReports: versioned(reviewForRevision, item.finalReviewReports, { source: 'worker' }),
      qualityReport: reviewForRevision,
      nodes: (item.nodes || []).map(node => !eligibleTargetIds.includes(node.id) ? node : ({
        ...node,
        status: 'node_revision_required',
