...
export const COURSE_WORKFLOW_STATUSES = [
  ...
  'assembly_pending', 'assembling', 'final_revision_required', 'final_review', 'final_review_human', 'completed', 'failed', 'paused', 'cancelled'
]
...
const DEFAULT_COURSE_SPEC = {
  ...
  qualityThreshold: 75, maxAutoRevisions: 2, maxTechnicalRetries: 2, reviewConcurrency: 2, promptVersion: 'course-controlled-v4-pipeline',
  models: {
...
function normalizedReviewDecision(report = {}, threshold = 75) {
  ...
  if (!scoresMeetThreshold(report, threshold)) return 'human_review'
  return 'approve'
}
...
function stageFraction(status) {
  return ({ preflight_required: .05, outline_pending: .12, outline_generating: .18, outline_review: .28, outline_approved: .36,
    ...
    assembly_pending: .76, assembling: .8, final_revision_required: .86, final_review: .88, final_review_human: .9, completed: 1 })[status] ?? 0
}
...
export function createInitialWorkflow({ textPack, courseName, teacher }) {
  ...
  return {
    ...
    lessons: (textPack.lessons || []).map(lesson => ({
      ...
      finalNoteVersions: [], finalReviewReports: [], finalRevisionRequests: [], qualityReport: null
    })),
...
export function assembleFinalNote(workflow, lessonKey, spliceData = {}, { trace = null } = {}) {
  ...
  const next = mapLesson(workflow, lessonKey, item => ({
    ...
    finalNote: { markdown, stale: false, updatedAt: nowIso(), assembly },
  ...
  return withDerivedProgress(setStep(setStep({ ...next, status: 'final_review_human', currentStep: 'final_review' }, 'assembly', 'done'), 'final_review', 'waiting_human'))
}
...
export function saveFinalNoteRevision(workflow, lessonKey, markdown, { trace = null } = {}) {
  ...
  const next = mapLesson(workflow, lessonKey, item => ({
    ...
    finalNoteVersions: versioned(value, item.finalNoteVersions, {
      ...
      trace,
      summary: '根据用户提交的最终修改要求生成。'
    })
  }))
  return withDerivedProgress(
    setStep(
      { ...next, status: 'final_review_human', currentStep: 'final_review' },
      'final_review',
      'waiting_human'
    )
  )
...
export function saveFinalNoteDraft(workflow, lessonKey, markdown) {
  ...
  const next = mapLesson(workflow, lessonKey, item => ({ ...item, status: 'final_review_human', qualityReport: null,
...
export function completeFinalReview(workflow, lessonKey, report = {}) {
  ...
  if (decision === 'revise') {
    ...
    if (!targetIds.length) {
      const humanReport = { ...qualityReport, decision: 'human_review', unmappedIssues: true }
      const next = mapLesson(workflow, lessonKey, item => ({
        ...item,
        status: 'final_review_human',
    ...
    const next = mapLesson(workflow, lessonKey, item => ({ ...item, status: 'node_revision_required',
      ...
      nodes: (item.nodes || []).map(node => !targetIds.includes(node.id) ? node : ({ ...node, status: 'node_revision_required', reviewDecision: 'revise',
