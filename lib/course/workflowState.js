    ...
    setStep(
      {
        ...
        progress: Math.max(next.progress || 0, 84)
      },
      'assembly',
      'done'
    ),
...
export function completeFinalReview(workflow, lessonKey, report = {}) {
  const lesson = firstLesson(workflow, lessonKey)
  if (!lesson.finalNote?.markdown) throw new Error('final note must be assembled before final review')
  const decision = report.decision || 'human_review'
  if (decision !== 'approve') {
    throw new Error('final review must approve before completion')
  }
  const qualityReport = {
    ...
    decision,
    checkedAt: nowIso(),
    assembledNodeCount: lesson.nodes?.length || 0
  }
  const next = mapLesson(workflow, lessonKey, item => ({
    ...item,
    status: 'completed',
    ...
    finalReviewReports: versioned(qualityReport, item.finalReviewReports),
    qualityReport
  }))
  const withReview = setStep(
    {
      ...next,
      status: 'completed',
  ...
  return setStep(withReview, 'completed', 'done')
}
