...
const DEFAULT_COURSE_SPEC = {
  ...
  qualityThreshold: 75, maxAutoRevisions: 2, promptVersion: 'course-controlled-v3-online',
  models: {
...
export function createInitialWorkflow({ textPack, courseName, teacher }) {
  ...
  return {
    ...
    steps: baseSteps(), pendingSteps: [], appliedTaskKeys: [], errors: [], activeErrorId: null, feedback: [], createdAt, updatedAt: createdAt
  }
}
...
export function applyNodeReview(workflow, lessonKey, nodeId, report = {}, { trace = null } = {}) {
  ...
  nextStatus = lessonStatus === 'node_revision_required' ? 'node_revision_required' : lessonStatus
  return withDerivedProgress(setStep({ ...next, status: nextStatus, currentStep: nextStatus === 'node_pending' ? 'node_generating' : 'node_review' }, 'node_review', nextStatus === 'node_revision_required' ? 'waiting_revision' : nextStatus === 'node_pending' ? 'pending' : 'waiting_human'))
}
...
function finishNodeApproval(workflow, lessonKey) {
  ...
  return withDerivedProgress(setStep({ ...synced, status, currentStep: status === 'assembly_pending' ? 'assembly' : status === 'node_pending' ? 'node_generating' : 'node_review' }, 'node_review', status === 'assembly_pending' ? 'done' : status === 'node_pending' ? 'pending' : 'waiting_human'))
}
...
export function failStep(workflow, { step, error, retryable = true, taskKey = '' }) {
  ...
  return { ...setStep(workflow, step, 'failed', { error: entry }), status: 'failed', resumeStatus: workflow.status, currentStep: step, activeErrorId: entry.id, errors, updatedAt: nowIso() }
}
