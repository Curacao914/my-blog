...
function copyFor(status, workflow, lesson, stats) {
  ...
  if (status === 'node_review') {
    ...
    return { stage: 'review', explanation: waiting ? '审查已经通过，请逐个确认节点内容。' : '正在审查正文。', primaryAction: waiting ? { type: 'open-review', label: '确认已通过内容' } : { type: 'refresh', label: '刷新审查进度' }, requiresHuman: waiting }
  }
