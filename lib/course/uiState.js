...
function stageIndex(status = '') {
  ...
  if (['assembly_pending', 'assembling', 'final_revision_required', 'final_review', 'final_review_human', 'note_removed'].includes(status)) return 5
...
function copyFor(status, workflow, lesson, stats) {
  ...
  if (status === 'note_removed') return { stage: 'assemble', explanation: '最终笔记已经永久删除；原始材料、大纲和节点正文仍然保留。', primaryAction: { type: 'regenerate-lesson-note', label: '重新生成笔记' }, requiresHuman: true }
  if (status === 'final_revision_required') return { stage: 'assemble', explanation: '正在按你的要求修改最终笔记。', primaryAction: { type: 'refresh', label: '查看修改进度' }, requiresHuman: false }
  if (status === 'final_review') return { stage: 'assemble', explanation: '最终笔记已经整理完成，正在进行自动最终检查。', primaryAction: { type: 'refresh', label: '查看检查进度' }, requiresHuman: false }
  if (status === 'final_review_human') {
    const reason = lesson?.finalReviewAttention?.message || '最终检查发现无法自动处理的重大异常。'
    return { stage: 'assemble', explanation: `需要人工处理：${reason}`, primaryAction: { type: 'approve-final-review', label: '查看异常并处理' }, blockedReason: reason, requiresHuman: true }
  }
