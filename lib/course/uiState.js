...
function copyFor(status, workflow, lesson, stats) {
  ...
  if (status === 'node_revision_required') return { stage: 'review', explanation: `有 ${stats.revisionNodes} 个节点正在排队修改，其他节点会继续处理。`, primaryAction: { type: 'refresh', label: '查看节点状态' }, requiresHuman: false }
  if (status === 'node_review') {
    return { stage: 'review', explanation: '正文会自动审查；通过后直接放行，实质问题会进入局部修改。', primaryAction: { type: 'refresh', label: '查看节点状态' }, requiresHuman: false }
  }
