...
const workerOnline = workflow => workflow?.worker?.status === 'online' && Number.isFinite(Date.parse(workflow.worker?.lastSeenAt || '')) && Date.now() - Date.parse(workflow.worker.lastSeenAt) < 120000
...
function copyFor(status, workflow, lesson, stats) {
  ...
  if (['preflight_approved', 'outline_pending', 'outline_generating'].includes(status)) return { stage: 'outline', explanation: '课程写作服务正在根据全部材料设计本课大纲。', primaryAction: { type: 'refresh', label: '刷新进度' }, blockedReason: workerOnline(workflow) ? '' : '请先连接本地处理服务', requiresHuman: false }
  ...
  if (['node_pending', 'node_generating'].includes(status)) return { stage: 'writing', explanation: `正在逐段整理正文，已完成 ${stats.approvedNodes}/${stats.totalNodes} 个节点。`, primaryAction: { type: 'refresh', label: '刷新进度' }, blockedReason: workerOnline(workflow) ? '' : '请先连接本地处理服务', requiresHuman: false }
