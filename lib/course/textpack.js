...
export function buildTextPack({ course = {}, preferences = {}, lessons = [], decks = [], warnings = [] }) {
  const normalizedLessons = lessons
    .map((lesson, index) => {
      ...
      return {
        ...
        title,
        sourceFile: safeName(lesson.sourceFile || lesson.fileName || `${title}.txt`),
        transcript,
        transcriptCharCount: transcript.length,
        transcriptLineCount: transcript ? transcript.split('\n').length : 0,
        segments: Array.isArray(lesson.segments)
          ? lesson.segments.map((segment, segmentIndex) => ({
              key: segment.key || `segment-${String(segmentIndex + 1).padStart(2, '0')}`,
              startLine: Number(segment.startLine || 1),
              endLine: Number(segment.endLine || segment.startLine || 1),
              charCount: Number(segment.charCount || 0)
            }))
  ...
  const pptText = decks.map((deck, index) => {
    ...
    return {
      ...
      lessonOrder: Number(deck.lessonOrder || deck.order || 1),
...
function assertNoRawPayload(value, path = 'textPack') {
  ...
  if (typeof value === 'string') {
    if (BASE64_BLOCK_PATTERN.test(value)) {
      throw new Error(`${path} 包含疑似 base64 或二进制内容，课程资料只允许纯文字。`)
    }
    if (SOURCE_PATH_PATTERN.test(value)) {
      throw new Error(`${path} 包含本地绝对路径，课程资料不能保存用户本地路径。`)
    }
    return
  }
...
export function validateTextPack(textPack) {
  if (!textPack || typeof textPack !== 'object') throw new Error('课程资料格式无效。')
  if (textPack.schemaVersion !== TEXTPACK_SCHEMA_VERSION) {
    throw new Error(`课程资料版本不受支持：${textPack.schemaVersion || '缺失版本信息'}`)
  }
  ...
  if (!lessons.length) throw new Error('至少需要一份可用的课程文字资料。')
