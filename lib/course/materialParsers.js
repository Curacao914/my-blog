...
export async function parseCourseMaterialFile(file, role = '') {
  ...
  return {
    ...
    role: role || (isMarkdown ? '已有笔记' : '补充材料'),
    lessonOrder,
...
export async function parseCourseMaterialFiles(files = []) {
  const materials = []
  const warnings = []
  for (const file of files) {
    try {
      materials.push(await parseCourseMaterialFile(file))
    } catch (error) {
...
export function materialsToTextPackInput({ materials, courseName, teacher }) {
  const transcripts = materials.filter(item => item.kind === 'transcript')
  const decks = materials.filter(item => item.kind === 'slides')
  const supplements = materials.filter(item => !['transcript', 'slides'].includes(item.kind))
  const primaryLessons = transcripts.length ? transcripts : supplements.slice(0, 1)
  return {
    course: { name: courseName, teacher },
    lessons: primaryLessons.map((item, index) => ({
      order: item.lessonOrder || index + 1,
      title: item.title,
      sourceFile: item.sourceFile,
      transcript: item.text,
      sourceMap: item.sourceMap || cleanText(item.text).split('\n').map((_, lineIndex) => ({
        line: lineIndex + 1,
        sourceLine: lineIndex + 1,
        file: item.sourceFile
      })),
      materials: supplements.filter(material => material.sourceFile !== item.sourceFile && (material.lessonOrder || 1) === (item.lessonOrder || 1)),
    ...
    warnings: [
      ...materials.flatMap(item => item.warnings || []),
      ...(!transcripts.length && supplements.length
        ? ['未识别到课堂转录，已将第一份文字资料作为本课正文。']
        : [])
    ]
  }
}
