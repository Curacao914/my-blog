...
function finalReviewIsCurrent(lesson) {
  ...
  if (report.decision === 'human_review' || report.autoRevisionExhausted === true) return false
  if (report.reviewedDraftVersion === undefined || report.reviewedDraftVersion === null) return false
...
function oneStageTask(workflow, lesson) {
  ...
  if (lesson.status === 'final_review' || workflow.status === 'final_review') {
    return finalReviewTask(workflow, lesson) || { type: 'idle', reason: 'final-review-current', lessonKey: lesson.key }
  }
