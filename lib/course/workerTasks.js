...
function revisionTask(workflow, lesson) {
  ...
  if (completedRevisions >= max || revision.humanReviewRequired) return null
  return {
    type: 'revise-node',
...
function nodeTaskBatch(workflow, lesson, { reviewConcurrency = 2, totalConcurrency = 3 } = {}) {
  const tasks = []
  // 写作与修订共用一条内容通道。优先修订，避免后文继续依赖已知有问题的前文。
  const contentTask = revisionTask(workflow, lesson) || writerTask(workflow, lesson)
  if (contentTask) tasks.push(contentTask)
  const remaining = Math.max(0, Number(totalConcurrency || 3) - tasks.length)
