...
function nodeWithNeighborContext(lesson, node) {
  ...
  return {
    ...
    writerBrief: {
      ...(node.writerBrief || {}),
      previousNodeSummary: previousDraft ? previousDraft.slice(0, 1200) : previous?.title || '',
...
function nodeTask(workflow, lesson) {
  ...
  if (revision) {
    ...
    return { type: 'revise-node', taskKey: `revise:${lesson.key}:${revision.id}:${count + 1}`, lessonKey: lesson.key, node: nodeWithNeighborContext(lesson, revision), courseSpec: workflow.courseSpec, lessonBlueprint: lesson.blueprint, outline: lesson.outline }
  }
  const pending = (lesson.nodes || []).find(node => node.status === 'node_pending')
  if (pending) return { type: 'write-node', taskKey: `write:${lesson.key}:${pending.id}:${(pending.versions || []).length + 1}`, lessonKey: lesson.key, node: nodeWithNeighborContext(lesson, pending), courseSpec: workflow.courseSpec, lessonBlueprint: lesson.blueprint, outline: lesson.outline }
  const review = (lesson.nodes || []).find(node => node.status === 'node_review' && (node.reviewRequired || !latestReview(node)))
  if (review) return { type: 'review-node', taskKey: `review:${lesson.key}:${review.id}:${(review.reviewerReports || []).length + 1}`, lessonKey: lesson.key, node: nodeWithNeighborContext(lesson, review), courseSpec: workflow.courseSpec, lessonBlueprint: lesson.blueprint, outline: lesson.outline }
