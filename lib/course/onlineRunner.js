...
export function normalizeCourseReviewScores(value = {}) {
  const keys = ['coverage', 'grounding', 'logic', 'detail', 'sourceCoverage']
  const scores = keys.map(key => Number(value[key]))
...
function validateReview(value) {
  ...
  normalizeCourseReviewScores(value)
...
async function repairOutlineGaps({ task, value, lineCount, initialTrace, modelConfig }) {
...
export async function executeCourseTask(task, options = {}) {
  ...
  if (task.type === 'revise-final-note') {
    ...
    const result = await callCourseModel({
      ...
      prompt: buildPrompt({
        ...
        lessonBlueprint: {
          title: task.lesson?.title,
          instruction: '只按用户要求修订当前完整笔记，返回完整 Markdown。'
        },
        writerBrief: {
          userRequest: request,
  ...
  if (task.type === 'generate-outline') {
    ...
    const repaired = await repairOutlineGaps({ task, value: initialValue, lineCount, initialTrace: result.trace, modelConfig: options.modelConfig })
