...
export async function listDueTaskReminders(ownerId, options = {}) {
...
export async function markTaskReminderSent(ownerId, taskId) {
  if (!ownerId) throw new Error('ownerId is required')
  const rows = await supabaseRequest(`/tasks?id=eq.${encodeURIComponent(taskId)}&owner_id=eq.${encodeURIComponent(ownerId)}`, {
    method: 'PATCH',
    headers: {
...
export async function dispatchTaskReminders(options = {}) {
  ...
  if (!ownerId) throw new Error('ownerId is required')
  const due = await listDueTaskReminders(ownerId, {
    now: options.now,
    limit: options.limit
  })
  ...
  if (shouldSend && due?.length) {
    ...
    if (shouldMark) {
      for (const task of due) {
        const updated = await markTaskReminderSent(ownerId, task.id)
