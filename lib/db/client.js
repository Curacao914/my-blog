...
export function getSupabaseStorageConfig(bucketOverride = '') {
  assertServerOnly()
  const config = assertDatabaseConfigured()
  const bucket = String(bucketOverride || '').trim() || config.storageBucket

  if (!config.supabaseUrl || !config.supabaseServiceRoleKey || !bucket) {
    throw new Error(
