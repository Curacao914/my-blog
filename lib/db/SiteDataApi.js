...
import { shouldSkipStaticPrefetch } from '../utils/staticPrefetchMode'
...
export async function fetchGlobalAllData({
  ...
  if (shouldSkipStaticPrefetch()) {
    console.log('[build] LAW_TECH_STATIC_PREFETCH_MODE=skip; skipping Notion static prefetch')
...
async function convertNotionToSiteData(
  ...
  collectionData.forEach(element => {
    if (
      element?.type === 'Post' &&
      !String(element.slug || '').trim() &&
      element.id
    ) {
      element.slug = String(element.id).replace(/-/g, '')
      element.slugGenerated = true
    }
