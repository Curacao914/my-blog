...
| Reminder runner | `pages/api/reminders/run.js`, `vercel.json` | Code prepared; production execution unverified | Uses `CRON_SECRET`, groups by owner, prevents same-date daily/weekly repeats, and is scheduled at `0 1 * * *` UTC. Preview requires a manual authenticated call. |
| Resend email reminders | `components/ReminderSettings.js`, `pages/api/reminders/*`, `lib/server/email.js` | Code prepared; migration/env/live send required | Apply `20260628_reminder_preferences.sql`; configure `RESEND_API_KEY`, verified `REMINDER_FROM`, and `CRON_SECRET`; save recipient in `/desk/system`, then send a test email. |
