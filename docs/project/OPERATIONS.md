...
- `SUPABASE_URL` 与 `SUPABASE_SERVICE_ROLE_KEY` 不得使用 targets 为 `Preview, Production` 的 Vercel 共享项；必须各有一条 Preview-only 和 Production-only 变量。修改后用 `vercel env ls` 核对四条独立记录，再对两个项目分别执行真实凭据探针。
