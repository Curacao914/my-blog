...
import { ensureProfile, getReminderPreferences } from '@/lib/server/supabase'
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const preference = await getReminderPreferences(profile.id)
    const requestedEmail = String(req.body?.email || '').trim()
    const to = requestedEmail || preference?.email || auth.user?.email || process.env.REMINDER_TO
