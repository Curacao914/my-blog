...
export default async function handler(req, res) {
  ...
  if (!serviceUrl || !process.env.LAW_TECH_OCR_SIGNING_SECRET) return res.status(503).json({ ok: false, error: '在线文字识别尚未配置' })
  try {
    const maxBytes = Math.min(100 * 1024 * 1024, Math.max(1, Number(req.body?.maxBytes || 100 * 1024 * 1024)))
    const token = createOcrUploadToken({ subject: auth.profile.id, maxBytes })
